...
using Celeste.Core.Platform.Save;
using Monocle;
...
public static class UserIO
{
	...
	{
		return Path.Combine(CelestePathBridge.ResolveSaveDirectory("Saves"), name + ".celeste");
	}

	private static string GetBackupHandle(string name)
	{
		return Path.Combine(CelestePathBridge.ResolveSaveDirectory(Path.Combine("Saves", "Backups")), name + ".celeste");
	}
	...
	public static bool Save<T>(string path, byte[] data) where T : class
	{
		...
		try
		{
			...
			AtomicFileWriter.WriteAllBytes(backupHandle, data);
			if (Load<T>(path, backup: true) != null)
			{
				AtomicFileWriter.WriteAllBytes(handle, data);
				flag = true;
			}
		}
		catch (Exception ex)
