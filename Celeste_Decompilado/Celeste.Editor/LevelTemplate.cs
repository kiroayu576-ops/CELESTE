...
public class LevelTemplate
{
	...
	public LevelTemplate(LevelData data)
	{
		...
		for (int i = 0; i < Height; i++)
		{
			...
			{
				int k;
				for (k = 0; j + k < Width && Back[j + k, i] && !Grid[j + k, i]; k++)
				{
				}
				if (k > 0)
				{
					backs.Add(new Rectangle(j, i, k, 1));
					j += k - 1;
				}
			}
			...
			{
				int m;
				for (m = 0; l + m < Width && Grid[l + m, i]; m++)
				{
				}
				if (m > 0)
				{
					solids.Add(new Rectangle(l, i, m, 1));
					l += m - 1;
				}
			}
		}
	...
	public void RenderContents(Camera camera, List<LevelTemplate> allLevels)
	{
		if (Type == LevelTemplateType.Level)
		{
			...
			{
				Draw.Rect((float)X + spawn.X, (float)Y + spawn.Y - 1f, 1f, 1f, Color.Red);
			}
	...
	public void Move(Vector2 relativeMove, List<LevelTemplate> allLevels, bool snap)
	{
		...
		foreach (LevelTemplate allLevel in allLevels)
		{
			...
			{
				bool num = Math.Abs(Left - allLevel.Right) < 3;
			...
			bool num2 = Math.Abs(Top - allLevel.Bottom) < 5;
			bool flag2 = Math.Abs(Bottom - allLevel.Top) < 5;
			if (num2)
			{
				Top = allLevel.Bottom;
			}
